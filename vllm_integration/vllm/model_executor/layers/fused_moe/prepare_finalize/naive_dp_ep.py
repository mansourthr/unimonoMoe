...
def _ep_combine(out, is_sp):
    """Timed wrapper for the combine collective."""
    with _ep_span("combine", "moe"):
        return get_ep_group().combine(out, is_sequence_parallel=is_sp)
...
class MoEPrepareAndFinalizeNaiveDPEPModular(mk.FusedMoEPrepareAndFinalizeModular):
    ...
    def finalize(
        ...
        output.copy_(_ep_combine(out, self.is_sequence_parallel))
